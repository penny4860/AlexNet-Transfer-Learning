...
def train(model, X_train, y_train, X_val, y_val, batch_size=100, n_epoches=5, ckpt=None):
    ...
    def _run_single_epoch(X_train, y_train, batch_size):
        total_cost = 0
        for offset, end in _get_batch_index(len(X_train), batch_size):
    ...
    with tf.Session() as sess:
        sess.run(init)
        total_batch = _get_n_batches(len(X_train), batch_size)
...
def evaluate(model, images, labels, session=None, ckpt=None, batch_size=100):
    ...
    def _evaluate(sess):
        if ckpt:
            _load_ckpt(sess, ckpt)

        accuracy_value = 0
        for offset, end in _get_batch_index(len(images), batch_size):
            accuracy_value += sess.run(model.accuracy_op,
                                       feed_dict={model.X: images[offset:end],
                                                  model.Y: labels[offset:end],
                                                  model.is_training: False})
            
        accuracy_value = accuracy_value / _get_n_batches(len(images), batch_size)
...
def inference(model, images, session=None, ckpt=None, batch_size=100):
    
    def _inference(sess):
        # todo : evaluate() 와 중복되는 부분을 extraction 하자.
        if ckpt:
            _load_ckpt(sess, ckpt)
        
        ys = []
        for offset, end in _get_batch_index(len(images), batch_size):
            labels = sess.run(model.inference_op,
                              feed_dict={model.X: images[offset:end],
                                         model.is_training: False})
            ys.append(labels)
...
def _load_ckpt(sess, ckpt):
    """restore checkpoint to session
    
    # Args
        sess : tf.Session instance
        ckpt : str
            directory name or checkpoint name
    """
    saver = tf.train.Saver()
...
def _get_batch_index(num_examples, batch_size=100):
...
def _get_n_batches(num_examples, batch_size=100):
